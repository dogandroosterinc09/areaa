(function () {
    ...
    /*
     * This js file will only contain members events
     *
     * */
    CPlatform.prototype.members = {

        initialize: function () {
            /* assign a value to the global variable within this class */
            uiMembersTable = $('#members-table');
            uiCreateMembersForm = $('#create-members');
            uiEditMembersForm = $('#edit-members');
            uiMembersDatatable = null;
            ...
            uiMembersDatatable = platform.members.initialize_datatable();

            /* create members validation */
            uiCreateMembersForm.find('[type="submit"]').on('click', function () {
            ...
            /* edit members validation */
            uiEditMembersForm.find('[type="submit"]').on('click', function () {
                if (platform.var_check(CKEDITOR) && platform.var_check(CKEDITOR.instances)) {
                    for (var instance in CKEDITOR.instances) {
                        CKEDITOR.instances[instance].updateElement();
                    }
            ...
            /* delete members button ajax */
            $('body').on('click', '.delete-members-btn', function (e) {
                e.preventDefault();
                var self = $(this);
                ...
                swal({
                    ...
                    if (isConfirm) {
                        var oParams = {
                            'data': {'id': self.attr('data-members-id')},
                            'url': self.attr('data-members-route')
                        };
                                    ...
                                    if (platform.var_check(oData.message)) {
                                        for (var x in oData.message) {
                                            ...
                                            swal({
                                                ...
                                                // $('[data-members-id="' + oData.data.id + '"]').remove();
                                                if (platform.var_check(uiMembersDatatable)) {
                                                    uiMembersDatatable.row(self.parents('tr:first')).remove();
                                                }

                                                uiMembersDatatable = platform.members.initialize_datatable();

                                                /* check if there are other members to hide the table header and show the no members found */
                                                if ($('tr[data-members-id]').length == 0) {
                                                    $('.members-empty').removeClass('johnCena');
        ...
        initialize_datatable: function () {
            if (platform.var_check(uiMembersDatatable)) {
                uiMembersDatatable.destroy();
            }

            /* members table initialize datatable */
