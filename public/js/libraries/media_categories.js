(function () {
    ...
    var uiMediaCategoriesTable,
        uiCreateMediaCategoryForm,
        uiEditMediaCategoryForm,
        uiMediaCategoriesDatatable,
        uiInputBannerImage,
        uiRemoveImgBtn,
        uiInputFile,
        uiRemoveFileBtn,
    ...
    /*
     * This js file will only contain media_category events
     *
     * */
    CPlatform.prototype.media_category = {

        initialize: function () {
            /* assign a value to the global variable within this class */
            uiMediaCategoriesTable = $('#media_categories-table');
            uiCreateMediaCategoryForm = $('#create-media_category');
            uiEditMediaCategoryForm = $('#edit-media_category');
            uiMediaCategoriesDatatable = null;
            ...
            uiMediaCategoriesDatatable = platform.media_category.initialize_datatable();

            /* create media_category validation */
            uiCreateMediaCategoryForm.find('[type="submit"]').on('click', function () {
            ...
            uiCreateMediaCategoryForm.validate({
            ...
            /* edit media_category validation */
            uiEditMediaCategoryForm.find('[type="submit"]').on('click', function () {
                if (platform.var_check(CKEDITOR) && platform.var_check(CKEDITOR.instances)) {
                    for (var instance in CKEDITOR.instances) {
                        CKEDITOR.instances[instance].updateElement();
                    }
            ...
            uiEditMediaCategoryForm.validate({
                errorClass: 'help-block animation-slideDown',
                errorElement: 'span',
                ignore: [':hidden:not([type="file"])'],
                errorPlacement: function (error, e) {
            ...
            /* delete media_category button ajax */
            $('body').on('click', '.delete-media_category-btn', function (e) {
                e.preventDefault();
                var self = $(this);
                ...
                swal({
                    ...
                    if (isConfirm) {
                        var oParams = {
                            'data': {'id': self.attr('data-media_category-id')},
                            'url': self.attr('data-media_category-route')
                        };
                                    ...
                                    if (platform.var_check(oData.message)) {
                                        for (var x in oData.message) {
                                            ...
                                            swal({
                                                ...
                                                /* remove media_category container */
                                                // $('[data-media_category-id="' + oData.data.id + '"]').remove();
                                                if (platform.var_check(uiMediaCategoriesDatatable)) {
                                                    uiMediaCategoriesDatatable.row(self.parents('tr:first')).remove();
                                                }

                                                uiMediaCategoriesDatatable = platform.media_category.initialize_datatable();

                                                /* check if there are other media_categories to hide the table header and show the no media_categories found */
                                                if ($('tr[data-media_category-id]').length == 0) {
                                                    $('.media_category-empty').removeClass('johnCena');
        ...
        initialize_datatable: function () {
            if (platform.var_check(uiMediaCategoriesDatatable)) {
                uiMediaCategoriesDatatable.destroy();
            }

            /* media_categories table initialize datatable */
            uiMediaCategoriesDatatable = uiMediaCategoriesTable.DataTable({
                "order": [[0, "asc"]],
                "paging": true,
                "pageLength": 10,
            ...
            return uiMediaCategoriesDatatable;
        },
    }
...
$(window).on('load', function () {
    platform.media_category.initialize();
});