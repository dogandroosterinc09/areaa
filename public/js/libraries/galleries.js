(function () {
    ...
    var uiGalleriesTable,
        uiCreateGalleryForm,
        uiEditGalleryForm,
        uiGalleriesDatatable,
        uiInputBannerImage,
        uiRemoveImgBtn,
        uiInputFile,
        uiRemoveFileBtn,
    ...
    /*
     * This js file will only contain gallery events
     *
     * */
    CPlatform.prototype.gallery = {

        initialize: function () {
            /* assign a value to the global variable within this class */
            uiGalleriesTable = $('#galleries-table');
            uiCreateGalleryForm = $('#create-gallery');
            uiEditGalleryForm = $('#edit-gallery');
            uiGalleriesDatatable = null;
            ...
            uiGalleriesDatatable = platform.gallery.initialize_datatable();

            /* create gallery validation */
            uiCreateGalleryForm.find('[type="submit"]').on('click', function () {
            ...
            /* edit gallery validation */
            uiEditGalleryForm.find('[type="submit"]').on('click', function () {
                if (platform.var_check(CKEDITOR) && platform.var_check(CKEDITOR.instances)) {
                    for (var instance in CKEDITOR.instances) {
                        CKEDITOR.instances[instance].updateElement();
                    }
            ...
            /* delete gallery button ajax */
            $('body').on('click', '.delete-gallery-btn', function (e) {
                e.preventDefault();
                var self = $(this);
                ...
                swal({
                    ...
                    if (isConfirm) {
                        var oParams = {
                            'data': {'id': self.attr('data-gallery-id')},
                            'url': self.attr('data-gallery-route')
                        };
                                    ...
                                    if (platform.var_check(oData.message)) {
                                        for (var x in oData.message) {
                                            ...
                                            swal({
                                                ...
                                                // $('[data-gallery-id="' + oData.data.id + '"]').remove();
                                                if (platform.var_check(uiGalleriesDatatable)) {
                                                    uiGalleriesDatatable.row(self.parents('tr:first')).remove();
                                                }

                                                uiGalleriesDatatable = platform.gallery.initialize_datatable();

                                                /* check if there are other galleries to hide the table header and show the no galleries found */
                                                if ($('tr[data-gallery-id]').length == 0) {
                                                    $('.gallery-empty').removeClass('johnCena');
        ...
        initialize_datatable: function () {
            if (platform.var_check(uiGalleriesDatatable)) {
                uiGalleriesDatatable.destroy();
            }

            /* galleries table initialize datatable */
            uiGalleriesDatatable = uiGalleriesTable.DataTable({
                "order": [[0, "asc"]],
                "paging": true,
                "pageLength": 10,
