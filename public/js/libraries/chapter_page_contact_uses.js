(function () {
    ...
    var uiChapterPageContactUsesTable,
        uiCreateChapterPageContactUsForm,
        uiEditChapterPageContactUsForm,
        uiChapterPageContactUsesDatatable,
        uiInputBannerImage,
        uiRemoveImgBtn,
        uiInputFile,
        uiRemoveFileBtn,
    ...
    /*
     * This js file will only contain chapter_page_contact_us events
     *
     * */
    CPlatform.prototype.chapter_page_contact_us = {

        initialize: function () {
            /* assign a value to the global variable within this class */
            uiChapterPageContactUsesTable = $('#chapter_page_contact_uses-table');
            uiCreateChapterPageContactUsForm = $('#create-chapter_page_contact_us');
            uiEditChapterPageContactUsForm = $('#edit-chapter_page_contact_us');
            uiChapterPageContactUsesDatatable = null;
            ...
            uiChapterPageContactUsesDatatable = platform.chapter_page_contact_us.initialize_datatable();

            /* create chapter_page_contact_us validation */
            uiCreateChapterPageContactUsForm.find('[type="submit"]').on('click', function () {
            ...
            uiCreateChapterPageContactUsForm.validate({
            ...
            /* edit chapter_page_contact_us validation */
            uiEditChapterPageContactUsForm.find('[type="submit"]').on('click', function () {
                if (platform.var_check(CKEDITOR) && platform.var_check(CKEDITOR.instances)) {
                    for (var instance in CKEDITOR.instances) {
                        CKEDITOR.instances[instance].updateElement();
                    }
            ...
            uiEditChapterPageContactUsForm.validate({
                errorClass: 'help-block animation-slideDown',
                errorElement: 'span',
                ignore: [':hidden:not([type="file"])'],
                errorPlacement: function (error, e) {
            ...
            /* delete chapter_page_contact_us button ajax */
            $('body').on('click', '.delete-chapter_page_contact_us-btn', function (e) {
                e.preventDefault();
                var self = $(this);
                ...
                swal({
                    ...
                    if (isConfirm) {
                        var oParams = {
                            'data': {'id': self.attr('data-chapter_page_contact_us-id')},
                            'url': self.attr('data-chapter_page_contact_us-route')
                        };
                                    ...
                                    if (platform.var_check(oData.message)) {
                                        for (var x in oData.message) {
                                            ...
                                            swal({
                                                ...
                                                /* remove chapter_page_contact_us container */
                                                // $('[data-chapter_page_contact_us-id="' + oData.data.id + '"]').remove();
                                                if (platform.var_check(uiChapterPageContactUsesDatatable)) {
                                                    uiChapterPageContactUsesDatatable.row(self.parents('tr:first')).remove();
                                                }

                                                uiChapterPageContactUsesDatatable = platform.chapter_page_contact_us.initialize_datatable();

                                                /* check if there are other chapter_page_contact_uses to hide the table header and show the no chapter_page_contact_uses found */
                                                if ($('tr[data-chapter_page_contact_us-id]').length == 0) {
                                                    $('.chapter_page_contact_us-empty').removeClass('johnCena');
        ...
        initialize_datatable: function () {
            if (platform.var_check(uiChapterPageContactUsesDatatable)) {
                uiChapterPageContactUsesDatatable.destroy();
            }

            /* chapter_page_contact_uses table initialize datatable */
            uiChapterPageContactUsesDatatable = uiChapterPageContactUsesTable.DataTable({
                "order": [[0, "asc"]],
                "paging": true,
                "pageLength": 10,
            ...
            return uiChapterPageContactUsesDatatable;
        },
    }
...
$(window).on('load', function () {
    platform.chapter_page_contact_us.initialize();
});