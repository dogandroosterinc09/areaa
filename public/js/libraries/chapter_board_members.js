(function () {
    ...
    var uiChapterBoardMembersTable,
        uiCreateChapterBoardMemberForm,
        uiEditChapterBoardMemberForm,
        uiChapterBoardMembersDatatable,
        uiInputBannerImage,
        uiRemoveImgBtn,
        uiInputFile,
        uiRemoveFileBtn,
    ...
    /*
     * This js file will only contain chapter_board_member events
     *
     * */
    CPlatform.prototype.chapter_board_member = {

        initialize: function () {
            /* assign a value to the global variable within this class */
            uiChapterBoardMembersTable = $('#chapter_board_members-table');
            uiCreateChapterBoardMemberForm = $('#create-chapter_board_member');
            uiEditChapterBoardMemberForm = $('#edit-chapter_board_member');
            uiChapterBoardMembersDatatable = null;
            ...
            uiChapterBoardMembersDatatable = platform.chapter_board_member.initialize_datatable();

            /* create chapter_board_member validation */
            uiCreateChapterBoardMemberForm.find('[type="submit"]').on('click', function () {
            ...
            uiCreateChapterBoardMemberForm.validate({
            ...
            /* edit chapter_board_member validation */
            uiEditChapterBoardMemberForm.find('[type="submit"]').on('click', function () {
                if (platform.var_check(CKEDITOR) && platform.var_check(CKEDITOR.instances)) {
                    for (var instance in CKEDITOR.instances) {
                        CKEDITOR.instances[instance].updateElement();
                    }
            ...
            uiEditChapterBoardMemberForm.validate({
                errorClass: 'help-block animation-slideDown',
                errorElement: 'span',
                ignore: [':hidden:not([type="file"])'],
                errorPlacement: function (error, e) {
            ...
            /* delete chapter_board_member button ajax */
            $('body').on('click', '.delete-chapter_board_member-btn', function (e) {
                e.preventDefault();
                var self = $(this);
                ...
                swal({
                    ...
                    if (isConfirm) {
                        var oParams = {
                            'data': {'id': self.attr('data-chapter_board_member-id')},
                            'url': self.attr('data-chapter_board_member-route')
                        };
                                    ...
                                    if (platform.var_check(oData.message)) {
                                        for (var x in oData.message) {
                                            ...
                                            swal({
                                                ...
                                                /* remove chapter_board_member container */
                                                // $('[data-chapter_board_member-id="' + oData.data.id + '"]').remove();
                                                if (platform.var_check(uiChapterBoardMembersDatatable)) {
                                                    uiChapterBoardMembersDatatable.row(self.parents('tr:first')).remove();
                                                }

                                                uiChapterBoardMembersDatatable = platform.chapter_board_member.initialize_datatable();

                                                /* check if there are other chapter_board_members to hide the table header and show the no chapter_board_members found */
                                                if ($('tr[data-chapter_board_member-id]').length == 0) {
                                                    $('.chapter_board_member-empty').removeClass('johnCena');
        ...
        initialize_datatable: function () {
            if (platform.var_check(uiChapterBoardMembersDatatable)) {
                uiChapterBoardMembersDatatable.destroy();
            }

            /* chapter_board_members table initialize datatable */
            uiChapterBoardMembersDatatable = uiChapterBoardMembersTable.DataTable({
                "order": [[0, "asc"]],
                "paging": true,
                "pageLength": 10,
            ...
            return uiChapterBoardMembersDatatable;
        },
    }
...
$(window).on('load', function () {
    platform.chapter_board_member.initialize();
});