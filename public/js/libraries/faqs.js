(function () {
    ...
    /*
     * This js file will only contain faq events
     *
     * */
    CPlatform.prototype.faq = {
                                    ...
                                    if (platform.var_check(oData.message)) {
                                        for (var x in oData.message) {
                                            ...
                                            swal({
                                                ...
                                                // $('[data-faq-id="' + oData.data.id + '"]').remove();
                                                ...
                                                /* check if there are other faqs to hide the table header and show the no faqs found */
        ...
        initialize_datatable: function () {
            ...
            /* faqs table initialize datatable */
            ...
            uiFaqsDatatable.on('row-reorder', function (e, diff, edit) {
                var data = [];
                for (var i = 0; i < diff.length; i++) {
                    var id = diff[i].node.dataset.faqId;
                ...
                platform.CentralAjax.ajax({
                    type: 'POST',
                    url: sAdminBaseURI + '/faqs/position',
                    data: {order: data},
                    token: platform.config('csrf.token'),
                    success: function () {
                    }
