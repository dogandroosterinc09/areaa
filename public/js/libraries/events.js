(function () {
    ...
    /*
     * This js file will only contain event events
     *
     * */
    CPlatform.prototype.event = {
        ...
        initialize: function () {
            ...
            uiEventsTable = $('#events-table');
            uiCreateEventForm = $('#create-event');
            uiEditEventForm = $('#edit-event');
            uiEventsDatatable = null;
            ...
            uiEventsDatatable = platform.event.initialize_datatable();
            ...
            uiEditEventForm.find('[type="submit"]').on('click', function () {
                if (platform.var_check(CKEDITOR) && platform.var_check(CKEDITOR.instances)) {
                    for (var instance in CKEDITOR.instances) {
                        CKEDITOR.instances[instance].updateElement();
                    }
                                    ...
                                    if (platform.var_check(oData.message)) {
                                        for (var x in oData.message) {
                                            ...
                                            swal({
                                                ...
                                                // $('[data-event-id="' + oData.data.id + '"]').remove();
                                                if (platform.var_check(uiEventsDatatable)) {
                                                    uiEventsDatatable.row(self.parents('tr:first')).remove();
                                                }

                                                uiEventsDatatable = platform.event.initialize_datatable();

                                                /* check if there are other events to hide the table header and show the no events found */
        ...
        initialize_datatable: function () {
            if (platform.var_check(uiEventsDatatable)) {
                uiEventsDatatable.destroy();
            }

            /* events table initialize datatable */
            uiEventsDatatable = uiEventsTable.DataTable({
                "order": [[0, "asc"]],
                "paging": true,
                "pageLength": 10,
