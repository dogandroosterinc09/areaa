(function () {
    ...
    var uiChapterPageEventsTable,
        uiCreateChapterPageEventForm,
        uiEditChapterPageEventForm,
        uiChapterPageEventsDatatable,
        uiInputBannerImage,
        uiRemoveImgBtn,
        uiInputFile,
        uiRemoveFileBtn,
    ...
    /*
     * This js file will only contain chapter_page_event events
     *
     * */
    CPlatform.prototype.chapter_page_event = {

        initialize: function () {
            /* assign a value to the global variable within this class */
            uiChapterPageEventsTable = $('#chapter_page_events-table');
            uiCreateChapterPageEventForm = $('#create-chapter_page_event');
            uiEditChapterPageEventForm = $('#edit-chapter_page_event');
            uiChapterPageEventsDatatable = null;
            ...
            uiChapterPageEventsDatatable = platform.chapter_page_event.initialize_datatable();

            /* create chapter_page_event validation */
            uiCreateChapterPageEventForm.find('[type="submit"]').on('click', function () {
            ...
            uiCreateChapterPageEventForm.validate({
            ...
            /* edit chapter_page_event validation */
            uiEditChapterPageEventForm.find('[type="submit"]').on('click', function () {
                if (platform.var_check(CKEDITOR) && platform.var_check(CKEDITOR.instances)) {
                    for (var instance in CKEDITOR.instances) {
                        CKEDITOR.instances[instance].updateElement();
                    }
            ...
            uiEditChapterPageEventForm.validate({
                errorClass: 'help-block animation-slideDown',
                errorElement: 'span',
                ignore: [':hidden:not([type="file"])'],
                errorPlacement: function (error, e) {
            ...
            /* delete chapter_page_event button ajax */
            $('body').on('click', '.delete-chapter_page_event-btn', function (e) {
                e.preventDefault();
                var self = $(this);
                ...
                swal({
                    ...
                    if (isConfirm) {
                        var oParams = {
                            'data': {'id': self.attr('data-chapter_page_event-id')},
                            'url': self.attr('data-chapter_page_event-route')
                        };
                                    ...
                                    if (platform.var_check(oData.message)) {
                                        for (var x in oData.message) {
                                            ...
                                            swal({
                                                ...
                                                /* remove chapter_page_event container */
                                                // $('[data-chapter_page_event-id="' + oData.data.id + '"]').remove();
                                                if (platform.var_check(uiChapterPageEventsDatatable)) {
                                                    uiChapterPageEventsDatatable.row(self.parents('tr:first')).remove();
                                                }

                                                uiChapterPageEventsDatatable = platform.chapter_page_event.initialize_datatable();

                                                /* check if there are other chapter_page_events to hide the table header and show the no chapter_page_events found */
                                                if ($('tr[data-chapter_page_event-id]').length == 0) {
                                                    $('.chapter_page_event-empty').removeClass('johnCena');
        ...
        initialize_datatable: function () {
            if (platform.var_check(uiChapterPageEventsDatatable)) {
                uiChapterPageEventsDatatable.destroy();
            }

            /* chapter_page_events table initialize datatable */
            uiChapterPageEventsDatatable = uiChapterPageEventsTable.DataTable({
                "order": [[0, "asc"]],
                "paging": true,
                "pageLength": 10,
            ...
            return uiChapterPageEventsDatatable;
        },
    }
...
$(window).on('load', function () {
    platform.chapter_page_event.initialize();
});