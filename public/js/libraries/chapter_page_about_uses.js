(function () {
    ...
    var uiChapterPageAboutUsesTable,
        uiCreateChapterPageAboutUsForm,
        uiEditChapterPageAboutUsForm,
        uiChapterPageAboutUsesDatatable,
        uiInputBannerImage,
        uiInputSection1FeaturedImage,
        uiInputSection2FeaturedImage,
        uiRemoveImgBtn,
        uiInputFile,
        uiRemoveFileBtn,
        filler;
    ...
    /*
     * This js file will only contain chapter_page_about_us events
     *
     * */
    CPlatform.prototype.chapter_page_about_us = {

        initialize: function () {
            /* assign a value to the global variable within this class */
            uiChapterPageAboutUsesTable = $('#chapter_page_about_uses-table');
            uiCreateChapterPageAboutUsForm = $('#create-chapter_page_about_us');
            uiEditChapterPageAboutUsForm = $('#edit-chapter_page_about_us');
            uiChapterPageAboutUsesDatatable = null;
            uiInputBannerImage = $('input[name="banner_image"]');
            uiInputSection1FeaturedImage = $('input[name="section_1_featured_image"]');
            uiInputSection2FeaturedImage = $('input[name="section_2_featured_image"]');
            uiInputFile = $('input[name="file"]');
            uiRemoveImgBtn = $('.remove-image-btn');
            uiRemoveFileBtn = $('.remove-file-btn');

            uiChapterPageAboutUsesDatatable = platform.chapter_page_about_us.initialize_datatable();

            /* create chapter_page_about_us validation */
            uiCreateChapterPageAboutUsForm.find('[type="submit"]').on('click', function () {
            ...
            uiCreateChapterPageAboutUsForm.validate({
            ...
            /* edit chapter_page_about_us validation */
            uiEditChapterPageAboutUsForm.find('[type="submit"]').on('click', function () {
                if (platform.var_check(CKEDITOR) && platform.var_check(CKEDITOR.instances)) {
                    for (var instance in CKEDITOR.instances) {
                        CKEDITOR.instances[instance].updateElement();
                    }
            ...
            uiEditChapterPageAboutUsForm.validate({
                errorClass: 'help-block animation-slideDown',
                errorElement: 'span',
                ignore: [':hidden:not([type="file"])'],
                errorPlacement: function (error, e) {
            ...
            /* delete chapter_page_about_us button ajax */
            $('body').on('click', '.delete-chapter_page_about_us-btn', function (e) {
                e.preventDefault();
                var self = $(this);
                ...
                swal({
                    ...
                    if (isConfirm) {
                        var oParams = {
                            'data': {'id': self.attr('data-chapter_page_about_us-id')},
                            'url': self.attr('data-chapter_page_about_us-route')
                        };
                                    ...
                                    if (platform.var_check(oData.message)) {
                                        for (var x in oData.message) {
                                            ...
                                            swal({
                                                ...
                                                /* remove chapter_page_about_us container */
                                                // $('[data-chapter_page_about_us-id="' + oData.data.id + '"]').remove();
                                                if (platform.var_check(uiChapterPageAboutUsesDatatable)) {
                                                    uiChapterPageAboutUsesDatatable.row(self.parents('tr:first')).remove();
                                                }

                                                uiChapterPageAboutUsesDatatable = platform.chapter_page_about_us.initialize_datatable();

                                                /* check if there are other chapter_page_about_uses to hide the table header and show the no chapter_page_about_uses found */
                                                if ($('tr[data-chapter_page_about_us-id]').length == 0) {
                                                    $('.chapter_page_about_us-empty').removeClass('johnCena');
            ...
            uiInputSection1FeaturedImage.on('change', function () {
                var input = $(this),
                    numFiles = input.get(0).files ? input.get(0).files.length : 1,
                    label = input.val().replace(/\\/g, '/').replace(/.*\//, ''),
                    sValue = $(this).val(),
            ...
            uiInputSection2FeaturedImage.on('change', function () {
                var input = $(this),
                    numFiles = input.get(0).files ? input.get(0).files.length : 1,
                    label = input.val().replace(/\\/g, '/').replace(/.*\//, ''),
                    sValue = $(this).val(),
        ...
        initialize_datatable: function () {
            if (platform.var_check(uiChapterPageAboutUsesDatatable)) {
                uiChapterPageAboutUsesDatatable.destroy();
            }

            /* chapter_page_about_uses table initialize datatable */
            uiChapterPageAboutUsesDatatable = uiChapterPageAboutUsesTable.DataTable({
                "order": [[0, "asc"]],
                "paging": true,
                "pageLength": 10,
            ...
            return uiChapterPageAboutUsesDatatable;
        },
    }
...
$(window).on('load', function () {
    platform.chapter_page_about_us.initialize();
});