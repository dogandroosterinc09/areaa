(function () {
    ...
    var uiChapterPageLeadershipsTable,
        uiCreateChapterPageLeadershipForm,
        uiEditChapterPageLeadershipForm,
        uiChapterPageLeadershipsDatatable,
        uiInputBannerImage,
        uiRemoveImgBtn,
        uiInputFile,
        uiRemoveFileBtn,
    ...
    /*
     * This js file will only contain chapter_page_leadership events
     *
     * */
    CPlatform.prototype.chapter_page_leadership = {

        initialize: function () {
            /* assign a value to the global variable within this class */
            uiChapterPageLeadershipsTable = $('#chapter_page_leaderships-table');
            uiCreateChapterPageLeadershipForm = $('#create-chapter_page_leadership');
            uiEditChapterPageLeadershipForm = $('#edit-chapter_page_leadership');
            uiChapterPageLeadershipsDatatable = null;
            ...
            uiChapterPageLeadershipsDatatable = platform.chapter_page_leadership.initialize_datatable();

            /* create chapter_page_leadership validation */
            uiCreateChapterPageLeadershipForm.find('[type="submit"]').on('click', function () {
            ...
            uiCreateChapterPageLeadershipForm.validate({
            ...
            /* edit chapter_page_leadership validation */
            uiEditChapterPageLeadershipForm.find('[type="submit"]').on('click', function () {
                if (platform.var_check(CKEDITOR) && platform.var_check(CKEDITOR.instances)) {
                    for (var instance in CKEDITOR.instances) {
                        CKEDITOR.instances[instance].updateElement();
                    }
            ...
            uiEditChapterPageLeadershipForm.validate({
                errorClass: 'help-block animation-slideDown',
                errorElement: 'span',
                ignore: [':hidden:not([type="file"])'],
                errorPlacement: function (error, e) {
            ...
            /* delete chapter_page_leadership button ajax */
            $('body').on('click', '.delete-chapter_page_leadership-btn', function (e) {
                e.preventDefault();
                var self = $(this);
                ...
                swal({
                    ...
                    if (isConfirm) {
                        var oParams = {
                            'data': {'id': self.attr('data-chapter_page_leadership-id')},
                            'url': self.attr('data-chapter_page_leadership-route')
                        };
                                    ...
                                    if (platform.var_check(oData.message)) {
                                        for (var x in oData.message) {
                                            ...
                                            swal({
                                                ...
                                                /* remove chapter_page_leadership container */
                                                // $('[data-chapter_page_leadership-id="' + oData.data.id + '"]').remove();
                                                if (platform.var_check(uiChapterPageLeadershipsDatatable)) {
                                                    uiChapterPageLeadershipsDatatable.row(self.parents('tr:first')).remove();
                                                }

                                                uiChapterPageLeadershipsDatatable = platform.chapter_page_leadership.initialize_datatable();

                                                /* check if there are other chapter_page_leaderships to hide the table header and show the no chapter_page_leaderships found */
                                                if ($('tr[data-chapter_page_leadership-id]').length == 0) {
                                                    $('.chapter_page_leadership-empty').removeClass('johnCena');
        ...
        initialize_datatable: function () {
            if (platform.var_check(uiChapterPageLeadershipsDatatable)) {
                uiChapterPageLeadershipsDatatable.destroy();
            }

            /* chapter_page_leaderships table initialize datatable */
            uiChapterPageLeadershipsDatatable = uiChapterPageLeadershipsTable.DataTable({
                "order": [[0, "asc"]],
                "paging": true,
                "pageLength": 10,
            ...
            return uiChapterPageLeadershipsDatatable;
        },
    }
...
$(window).on('load', function () {
    platform.chapter_page_leadership.initialize();
});