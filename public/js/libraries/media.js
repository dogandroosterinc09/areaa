(function () {
    ...
    /*
     * This js file will only contain media events
     *
     * */
    CPlatform.prototype.media = {

        initialize: function () {
            /* assign a value to the global variable within this class */
            uiMediaTable = $('#media-table');
            uiCreateMediaForm = $('#create-media');
            uiEditMediaForm = $('#edit-media');
            uiMediaDatatable = null;
            ...
            uiMediaDatatable = platform.media.initialize_datatable();

            /* create media validation */
            uiCreateMediaForm.find('[type="submit"]').on('click', function () {
            ...
            uiEditMediaForm.find('[type="submit"]').on('click', function () {
                if (platform.var_check(CKEDITOR) && platform.var_check(CKEDITOR.instances)) {
                    for (var instance in CKEDITOR.instances) {
                        CKEDITOR.instances[instance].updateElement();
                    }
            ...
            /* delete media button ajax */
            $('body').on('click', '.delete-media-btn', function (e) {
                e.preventDefault();
                var self = $(this);
                ...
                swal({
                    ...
                    if (isConfirm) {
                        var oParams = {
                            'data': {'id': self.attr('data-media-id')},
                            'url': self.attr('data-media-route')
                        };
                                    ...
                                    if (platform.var_check(oData.message)) {
                                        for (var x in oData.message) {
                                            ...
                                            swal({
                                                ...
                                                // $('[data-media-id="' + oData.data.id + '"]').remove();
                                                if (platform.var_check(uiMediaDatatable)) {
                                                    uiMediaDatatable.row(self.parents('tr:first')).remove();
                                                }

                                                uiMediaDatatable = platform.media.initialize_datatable();

                                                /* check if there are other media to hide the table header and show the no media found */
                                                if ($('tr[data-media-id]').length == 0) {
                                                    $('.media-empty').removeClass('johnCena');
        ...
        initialize_datatable: function () {
            if (platform.var_check(uiMediaDatatable)) {
                uiMediaDatatable.destroy();
            }

            /* media table initialize datatable */
            uiMediaDatatable = uiMediaTable.DataTable({
                "order": [[0, "asc"]],
                "paging": true,
                "pageLength": 10,
