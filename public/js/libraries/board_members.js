(function () {
    ...
    var uiBoardMembersTable,
        uiCreateBoardMemberForm,
        uiEditBoardMemberForm,
        uiBoardMembersDatatable,
        uiInputBannerImage,
        uiRemoveImgBtn,
        uiInputFile,
        uiRemoveFileBtn,
    ...
    /*
     * This js file will only contain board_member events
     *
     * */
    CPlatform.prototype.board_member = {

        initialize: function () {
            /* assign a value to the global variable within this class */
            uiBoardMembersTable = $('#board_members-table');
            uiCreateBoardMemberForm = $('#create-board_member');
            uiEditBoardMemberForm = $('#edit-board_member');
            uiBoardMembersDatatable = null;
            ...
            uiBoardMembersDatatable = platform.board_member.initialize_datatable();

            /* create board_member validation */
            uiCreateBoardMemberForm.find('[type="submit"]').on('click', function () {
            ...
            uiCreateBoardMemberForm.validate({
            ...
            /* edit board_member validation */
            uiEditBoardMemberForm.find('[type="submit"]').on('click', function () {
                if (platform.var_check(CKEDITOR) && platform.var_check(CKEDITOR.instances)) {
                    for (var instance in CKEDITOR.instances) {
                        CKEDITOR.instances[instance].updateElement();
                    }
            ...
            uiEditBoardMemberForm.validate({
                errorClass: 'help-block animation-slideDown',
                errorElement: 'span',
                ignore: [':hidden:not([type="file"])'],
                errorPlacement: function (error, e) {
            ...
            /* delete board_member button ajax */
            $('body').on('click', '.delete-board_member-btn', function (e) {
                e.preventDefault();
                var self = $(this);
                ...
                swal({
                    ...
                    if (isConfirm) {
                        var oParams = {
                            'data': {'id': self.attr('data-board_member-id')},
                            'url': self.attr('data-board_member-route')
                        };
                                    ...
                                    if (platform.var_check(oData.message)) {
                                        for (var x in oData.message) {
                                            ...
                                            swal({
                                                ...
                                                /* remove board_member container */
                                                // $('[data-board_member-id="' + oData.data.id + '"]').remove();
                                                if (platform.var_check(uiBoardMembersDatatable)) {
                                                    uiBoardMembersDatatable.row(self.parents('tr:first')).remove();
                                                }

                                                uiBoardMembersDatatable = platform.board_member.initialize_datatable();

                                                /* check if there are other board_members to hide the table header and show the no board_members found */
                                                if ($('tr[data-board_member-id]').length == 0) {
                                                    $('.board_member-empty').removeClass('johnCena');
        ...
        initialize_datatable: function () {
            if (platform.var_check(uiBoardMembersDatatable)) {
                uiBoardMembersDatatable.destroy();
            }

            /* board_members table initialize datatable */
            uiBoardMembersDatatable = uiBoardMembersTable.DataTable({
                "order": [[0, "asc"]],
                "paging": true,
                "pageLength": 10,
            ...
            uiBoardMembersDatatable.on('row-reorder', function (e, diff, edit) {
                var data = [];
                for (var i = 0; i < diff.length; i++) {
                    var id = diff[i].node.dataset.board_memberId;
                    var position = ++diff[i].newPosition;
                    data.push({id, position});
                }

                platform.CentralAjax.ajax({
                    type: 'POST',
                    url: sAdminBaseURI + '/board_members/position',
                    data: {order: data},
                    token: platform.config('csrf.token'),
                    success: function () {
                    }
            ...
            return uiBoardMembersDatatable;
        },
    }
...
$(window).on('load', function () {
    platform.board_member.initialize();
});