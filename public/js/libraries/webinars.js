(function () {
    ...
    /*
     * This js file will only contain webinars events
     *
     * */
    CPlatform.prototype.webinars = {

        initialize: function () {
            /* assign a value to the global variable within this class */
            uiWebinarsTable = $('#webinars-table');
            uiCreateWebinarsForm = $('#create-webinars');
            ...
            uiWebinarsDatatable = platform.webinars.initialize_datatable();

            /* create webinars validation */
            uiCreateWebinarsForm.find('[type="submit"]').on('click', function () {
            ...
            /* edit webinars validation */
            uiEditWebinarsForm.find('[type="submit"]').on('click', function () {
                if (platform.var_check(CKEDITOR) && platform.var_check(CKEDITOR.instances)) {
                    for (var instance in CKEDITOR.instances) {
                        CKEDITOR.instances[instance].updateElement();
                    }
            ...
            /* delete webinars button ajax */
            $('body').on('click', '.delete-webinars-btn', function (e) {
                e.preventDefault();
                var self = $(this);
                ...
                swal({
                    ...
                    if (isConfirm) {
                        var oParams = {
                            'data': {'id': self.attr('data-webinars-id')},
                            'url': self.attr('data-webinars-route')
                        };
                                    ...
                                    if (platform.var_check(oData.message)) {
                                        for (var x in oData.message) {
                                            ...
                                            swal({
                                                ...
                                                /* remove webinars container */
                                                // $('[data-webinars-id="' + oData.data.id + '"]').remove();
                                                if (platform.var_check(uiWebinarsDatatable)) {
                                                    uiWebinarsDatatable.row(self.parents('tr:first')).remove();
                                                }

                                                uiWebinarsDatatable = platform.webinars.initialize_datatable();

                                                /* check if there are other webinars to hide the table header and show the no webinars found */
                                                if ($('tr[data-webinars-id]').length == 0) {
                                                    $('.webinars-empty').removeClass('johnCena');
        ...
        initialize_datatable: function () {
            if (platform.var_check(uiWebinarsDatatable)) {
                uiWebinarsDatatable.destroy();
            }

            /* webinars table initialize datatable */
            uiWebinarsDatatable = uiWebinarsTable.DataTable({
                "order": [[0, "asc"]],
                "paging": true,
                "pageLength": 10,
