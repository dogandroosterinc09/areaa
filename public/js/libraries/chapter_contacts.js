(function () {
    ...
    var uiChapterContactsTable,
        uiCreateChapterContactForm,
        uiEditChapterContactForm,
        uiChapterContactsDatatable,
        uiInputBannerImage,
        uiRemoveImgBtn,
        uiInputFile,
        uiRemoveFileBtn,
    ...
    /*
     * This js file will only contain chapter_contact events
     *
     * */
    CPlatform.prototype.chapter_contact = {

        initialize: function () {
            /* assign a value to the global variable within this class */
            uiChapterContactsTable = $('#chapter_contacts-table');
            uiCreateChapterContactForm = $('#create-chapter_contact');
            uiEditChapterContactForm = $('#edit-chapter_contact');
            uiChapterContactsDatatable = null;
            ...
            uiChapterContactsDatatable = platform.chapter_contact.initialize_datatable();

            /* create chapter_contact validation */
            uiCreateChapterContactForm.find('[type="submit"]').on('click', function () {
            ...
            uiCreateChapterContactForm.validate({
            ...
            /* edit chapter_contact validation */
            uiEditChapterContactForm.find('[type="submit"]').on('click', function () {
                if (platform.var_check(CKEDITOR) && platform.var_check(CKEDITOR.instances)) {
                    for (var instance in CKEDITOR.instances) {
                        CKEDITOR.instances[instance].updateElement();
                    }
            ...
            uiEditChapterContactForm.validate({
                errorClass: 'help-block animation-slideDown',
                errorElement: 'span',
                ignore: [':hidden:not([type="file"])'],
                errorPlacement: function (error, e) {
            ...
            /* delete chapter_contact button ajax */
            $('body').on('click', '.delete-chapter_contact-btn', function (e) {
                e.preventDefault();
                var self = $(this);
                ...
                swal({
                    ...
                    if (isConfirm) {
                        var oParams = {
                            'data': {'id': self.attr('data-chapter_contact-id')},
                            'url': self.attr('data-chapter_contact-route')
                        };
                                    ...
                                    if (platform.var_check(oData.message)) {
                                        for (var x in oData.message) {
                                            ...
                                            swal({
                                                ...
                                                /* remove chapter_contact container */
                                                // $('[data-chapter_contact-id="' + oData.data.id + '"]').remove();
                                                if (platform.var_check(uiChapterContactsDatatable)) {
                                                    uiChapterContactsDatatable.row(self.parents('tr:first')).remove();
                                                }

                                                uiChapterContactsDatatable = platform.chapter_contact.initialize_datatable();

                                                /* check if there are other chapter_contacts to hide the table header and show the no chapter_contacts found */
                                                if ($('tr[data-chapter_contact-id]').length == 0) {
                                                    $('.chapter_contact-empty').removeClass('johnCena');
        ...
        initialize_datatable: function () {
            if (platform.var_check(uiChapterContactsDatatable)) {
                uiChapterContactsDatatable.destroy();
            }

            /* chapter_contacts table initialize datatable */
            uiChapterContactsDatatable = uiChapterContactsTable.DataTable({
                "order": [[0, "asc"]],
                "paging": true,
                "pageLength": 10,
            ...
            return uiChapterContactsDatatable;
        },
    }
...
$(window).on('load', function () {
    platform.chapter_contact.initialize();
});