(function () {
    ...
    var uiChapterEventsTable,
        uiCreateChapterEventForm,
        uiEditChapterEventForm,
        uiChapterEventsDatatable,
        uiInputBannerImage,
        uiRemoveImgBtn,
        uiInputFile,
        uiRemoveFileBtn,
    ...
    /*
     * This js file will only contain chapter_event events
     *
     * */
    CPlatform.prototype.chapter_event = {

        initialize: function () {
            /* assign a value to the global variable within this class */
            uiChapterEventsTable = $('#chapter_events-table');
            uiCreateChapterEventForm = $('#create-chapter_event');
            uiEditChapterEventForm = $('#edit-chapter_event');
            uiChapterEventsDatatable = null;
            ...
            uiChapterEventsDatatable = platform.chapter_event.initialize_datatable();

            /* create chapter_event validation */
            uiCreateChapterEventForm.find('[type="submit"]').on('click', function () {
            ...
            uiCreateChapterEventForm.validate({
            ...
            /* edit chapter_event validation */
            uiEditChapterEventForm.find('[type="submit"]').on('click', function () {
                if (platform.var_check(CKEDITOR) && platform.var_check(CKEDITOR.instances)) {
                    for (var instance in CKEDITOR.instances) {
                        CKEDITOR.instances[instance].updateElement();
                    }
            ...
            uiEditChapterEventForm.validate({
                errorClass: 'help-block animation-slideDown',
                errorElement: 'span',
                ignore: [':hidden:not([type="file"])'],
                errorPlacement: function (error, e) {
            ...
            /* delete chapter_event button ajax */
            $('body').on('click', '.delete-chapter_event-btn', function (e) {
                e.preventDefault();
                var self = $(this);
                ...
                swal({
                    ...
                    if (isConfirm) {
                        var oParams = {
                            'data': {'id': self.attr('data-chapter_event-id')},
                            'url': self.attr('data-chapter_event-route')
                        };
                                    ...
                                    if (platform.var_check(oData.message)) {
                                        for (var x in oData.message) {
                                            ...
                                            swal({
                                                ...
                                                /* remove chapter_event container */
                                                // $('[data-chapter_event-id="' + oData.data.id + '"]').remove();
                                                if (platform.var_check(uiChapterEventsDatatable)) {
                                                    uiChapterEventsDatatable.row(self.parents('tr:first')).remove();
                                                }

                                                uiChapterEventsDatatable = platform.chapter_event.initialize_datatable();

                                                /* check if there are other chapter_events to hide the table header and show the no chapter_events found */
                                                if ($('tr[data-chapter_event-id]').length == 0) {
                                                    $('.chapter_event-empty').removeClass('johnCena');
        ...
        initialize_datatable: function () {
            if (platform.var_check(uiChapterEventsDatatable)) {
                uiChapterEventsDatatable.destroy();
            }

            /* chapter_events table initialize datatable */
            uiChapterEventsDatatable = uiChapterEventsTable.DataTable({
                "order": [[0, "asc"]],
                "paging": true,
                "pageLength": 10,
            ...
            return uiChapterEventsDatatable;
        },
    }
...
$(window).on('load', function () {
    platform.chapter_event.initialize();
});